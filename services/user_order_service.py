...
def create_user_order(ticker, order_type, quantity, price):
    ...
    if order_type not in ["BUY", "SELL"]:
        return False, "Order type must be BUY or SELL.", None

    if not isinstance(quantity, int) or quantity <= 0:
        return False, "Quantity must be a positive whole number.", None
    ...
    if order_type == "SELL":
        ...
        if not portfolio:
            return False, "You do not own any shares of this stock.", None
        ...
        if available_quantity < quantity:
            return False, "You do not have enough unreserved shares.", None

    if order_type == "BUY":
        total_cost = price * quantity

        available_cash = (
            user.cash_balance - user.reserved_cash
        )

        if available_cash < total_cost:
            return False, "Insufficient available funds.", None
    ...
    log_audit_action(
        ...
        f"{quantity} shares of {stock.ticker} at ${price:.2f} per share."
    )
...
def update_user_order(order_id, quantity, price):
    user_id = session.get("user_id")
    ...
    if order.user_id != user.id:
        return False, "You can only modify your own orders."
    ...
    if order.order_type == "SELL":
        ...
        if not portfolio:
            return False, "Portfolio entry not found."
        ...
        user.reserved_cash -= old_reserved
        ...
        user.reserved_cash += new_total_cost
    ...
    log_audit_action(
        ...
        f"{order.order_type} {quantity} shares of {stock.ticker} "
...
def cancel_user_order(order_id):
    ...
    if order.order_type == "SELL":
        ...
        user.reserved_cash -= order.quantity * order.price
...
def accept_user_order(order_id):
    accepting_user_id = session.get("user_id")

    if not accepting_user_id:
        return False, "You must be logged in."

    accepting_user = db.session.get(User, accepting_user_id)
    ...
    order_creator = db.session.get(User, order.user_id)

    if not order_creator:
        return False, "Order creator not found."
    ...
    try:
        ...
        if order.order_type == "SELL":
            ...
            if not seller_portfolio:
                return False, "Seller does not own this stock."

            if seller_portfolio.reserved_quantity < order.quantity:
                return False, "Seller's reserved shares are invalid."

            available_buyer_cash = (
                buyer.cash_balance - buyer.reserved_cash
            )
            ...
            if not buyer_portfolio:
                ...
                db.session.add(buyer_portfolio)
            ...
            seller_portfolio.reserved_quantity -= order.quantity

            # Transfer shares.
            seller_portfolio.quantity -= order.quantity
            buyer_old_quantity = buyer_portfolio.quantity

            # Calculate buyer's new average cost.
            buyer_old_value = (
                float(buyer_portfolio.avg_buy_price)
                * buyer_old_quantity
            )

            buyer_portfolio.quantity += order.quantity

            buyer_portfolio.avg_buy_price = (
                buyer_old_value + float(total_value)
        ...
        #
            ...
            if available_seller_quantity < order.quantity:
                return False, "Seller does not have enough unreserved shares."

            # The BUY order creator already reserved the money.
            if buyer.reserved_cash < total_value:
                return False, "Buy order reservation is invalid."
            ...
            buyer.reserved_cash -= total_value
            buyer.cash_balance -= total_value
        ...
        order.accepted_by_user_id = accepting_user.id
        order.accepted_at = db.func.current_timestamp()
